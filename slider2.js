function Slider(settings) {
  ...
  var sliderContent = slider.querySelector(".slider-content"); // контейнер слайдера, который мы ищем внутри слайдера sliderGallery, [0] первый элемент из коллекции
  var sliderItems = slider.querySelectorAll(".slider2__slide"); // выбор всех слайдов т.е. айтемов в слайдере
  ...
  function getItemsClone() {
    ...
    for (var i = sliderItems.length - 1; i >= 0; i--) {
      var clone = sliderItems[i].cloneNode(true);
    ...
    for (var i = 0; i < sliderItems.length; i++) {
      ...
      sliderContent.append(sliderItems[i].cloneNode(true));
    }    

    sliderItems = slider.querySelectorAll(".slider2__slide");
  }
